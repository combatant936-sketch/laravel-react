...
function PostsEdit() {
    ...
    const submit = (e) => {
        ...
        axios.post("/api/posts/" + params.id, formData1).then((res) => { Swal.fire("Success", "Post updated successfully", "success"); navigate("/") })
            .catch((error) => { setErrors(prev => ({ ...prev, ...error.response.data.errors })); console.log(error); Swal.fire("Error", error.response.data.message, "error") }).finally(() => setIsLoading(() => false))
    }
